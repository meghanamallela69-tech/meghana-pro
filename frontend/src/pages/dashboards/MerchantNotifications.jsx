...
import {
  ...
  FaDollarSign, FaSearch, FaCheckCircle, FaTimesCircle,
  FaExclamationCircle, FaArrowRight, FaUserPlus
} from "react-icons/fa";
...
const TYPE_META = {
  booking:              { icon: FaCalendarAlt,      bg: "bg-green-100",  text: "text-green-600",  label: "New Booking" },
  booking_request:      { icon: FaUserPlus,         bg: "bg-purple-100", text: "text-purple-600", label: "Booking Request" },
  ...
  payment_received:     { icon: FaDollarSign,        bg: "bg-green-100",  text: "text-green-600",  label: "Payment Received" },
...
const MerchantNotifications = () => {
  ...
  const getActionLink = (notif) => {
    if (notif.actionUrl) return () => navigate(notif.actionUrl);
    if (notif.type?.includes("booking")) return () => navigate("/dashboard/merchant/bookings");
    if (notif.type?.includes("payment")) return () => navigate("/dashboard/merchant/earnings");
    return null;
  };

  const getActionLabel = (notif) => {
    if (notif.type?.includes("booking")) return "View Bookings";
    if (notif.type?.includes("payment")) return "View Earnings";
    return "View";
  };
    ...
    <MerchantLayout>
      ...
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Booking requests, payments and updates</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-400 text-sm mt-1">Booking requests and payment updates will appear here</p>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {action && (
                      ...
                      >
                        {getActionLabel(notif)} <FaArrowRight size={10} />
                      </button>
                    )}
                  </div>
